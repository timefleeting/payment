...
class Utils
{
    ...
    public static function mapData(array $mode, $data, array $alias = [])
    {
        if (!is_array($data)) {
            return $mode;
        }
        ...
        foreach ($mode as $field => $val) {
            if (!empty($val) && is_array($val)) {
                $v = self::mapData($val, (isset($data[$field]) ? $data[$field] : []), (isset($alias[$field]) ? $alias[$field] : []));
            } else {
                if (!empty($alias[$field])) {
                    ...
                    $v = isset($data[$field]) ? $data[$field] : $val;
                    //为空，默认以mode值为准
                    if (empty($v)) {
                        $v = $val;
                    }
     ...
     *
     * @param string $pre 前缀
     * @param string $name 驼峰，首写母大写
     * @return void
     */
    public static function camelcaseName($pre, $name)
    {
        return $pre . str_replace(' ', '', ucwords(str_replace(array('_', '-'), ' ', $name)));
    }

    /**
     * 模拟表单提交
     */
    public static function formSubmit($url, $data)
    {
        if (empty($url)) {
            return '';
        }
        $formHtml = '<form id="formSubmit" action="' . $url . '" name="formSubmit" method="POST" >';
        if (!empty($data)&&is_array($data)) {
            foreach($data as $k => $val) {
                $formHtml .= '<input type="hidden" name="'.$k.'"  value="' . $val . '">';
            }
        }
        $formHtml .= '</form>';
        $formHtml .= '<script>document.getElementById("formSubmit").submit()</script>';
        return $formHtml;
    }
