 ...
 * @Last Modified time: 2022-09-27 17:18:32
 * 通用请口接口类
 */
...
class Request
{
     ...
     * [
     *      'type' => 'api',
     *      'url'  => 'Closure 匿名函数对象', //return 必须是Response对象数据
     *      'data' =>  'Closure 函数参数'
     *      'log'  =>  '记录日志类'
     *      'title' => '日志文件标题'
     * ]
     */
    public static function apiCreate(RequestCreate $Params, Closure $Fn = null)
    {
        $api = $Params->getUrl();
        ...
        $startTime = microtime(true);
        $startMem = memory_get_usage();
        $ret = call_user_func_array($api, ['data' => $Params->getData()]);
        $Response = new Response($ret);
        $runTime = round(microtime(true) - $startTime, 10); // s
        $memoryUse = number_format((memory_get_usage() - $startMem) / 1024, 2); // kb
        $Response->setRunTime($runTime)->setMemoryUse($memoryUse);
     ...
     * @return array
     */
    public static function receive($type = '', $rawData = [])
    {
        $get   = $_GET;
        $post  = $_POST;
        $request  = $_REQUEST;
        ...
        switch ($type) {
            ...
            break;
            default:
               return array_merge($request, $put); 
            break;
        }
    }
    /**
