 ...
 * @Last Modified time: 2022-06-16 00:10:31
 */
...
class Model implements \JsonSerializable, \ArrayAccess, \IteratorAggregate, \Countable
{
    ...
    //$fields字段值为null是否可忽略, 0忽略;1,不允许忽略必填有值(除了null以外的值); 2按类型填充
     ...
     *
     * @param array $data
     * @param bool nullRequire 是否null字段跳过, 默认1不跳过
     */
    public function __construct(array $data = [], $nullRequire = 1)
    {
        $this->nullRequire = $nullRequire;
    ...
    protected function setterMode(array $mode, array $data = [], array $defaults = [], $flag = [])
    {
        ...
        foreach ($mode as $k => $type) {
            ...
            $val = array_key_exists($k, $data) ? (is_object($data[$k]) ? clone $data[$k] : $data[$k]) : (array_key_exists($k, $defaults) ? (is_object($defaults[$k]) ? clone $defaults[$k] : $defaults[$k]) : null);
    ...
    protected function modeProperty($key, $type, $val)
    {
        ...
        if (is_null($val)) {
            ...
            } elseif ($this->nullRequire ==0) {
                return false; //continue; 不允许null值,直接跳过
            } elseif ($this->nullRequire == 2) {
                //填充
                switch ($type) {
                    case 'string':
                        $val = '';
